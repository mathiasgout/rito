...
class ChampionMasteryExtractor(BaseExtractor):
    def extract(self, champion_mastery_dict: dict) -> ChampionMastery:
        if not isinstance(champion_mastery_dict, dict):
            raise ExtractorError(f"type(champion_mastery_dict)={type(champion_mastery_dict)} (!= dict)")
        ...
        champion_mastery = ChampionMastery(
            ...
            champion_points_since_last_level=champion_mastery_dict.get("championPointsSinceLastLevel", None),
            champion_points_until_next_level=champion_mastery_dict.get("championPointsUntilNextLevel", None),
    ...
    def extract_from_list(self, champion_masteries_list: list[dict], champion_id: str) -> ChampionMastery:
        if not isinstance(champion_masteries_list, list):
            raise ExtractorError(f"type(champion_masteries_list)={type(champion_masteries_list)} (!= list of dictionnaries)")
        if not champion_id:
    ...
    def extract_totals_from_list(self, champion_masteries_list: list[dict]) -> ChampionMasteryTotals:
        if not isinstance(champion_masteries_list, list):
            raise ExtractorError(f"type(champion_masteries_list)={type(champion_masteries_list)} (!= list of dictionnaries)")
    ...
    @staticmethod
    def _get_champion_mastery_by_champion_id(champion_masteries_list: list[dict], champion_id: str) -> dict:
        for champion_mastery_dict in champion_masteries_list:
            if champion_mastery_dict.get("championId", None) == int(champion_id):
                return champion_mastery_dict
        raise ExtractorError(f"champion with champion_id={champion_id} not in champion_masteries_list")
